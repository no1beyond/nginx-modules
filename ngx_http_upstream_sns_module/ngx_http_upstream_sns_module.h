...
#define NGX_HTTP_LB_IP_HASH        (1<<1)
#define NGX_HTTP_LB_LEAST_CONN     (1<<2)
#define NGX_HTTP_LB_HASH_MODULA    (1<<3)
#define NGX_HTTP_LB_HASH_KETAMA    (1<<4)
#define NGX_HTTP_LB_NODEID         (1<<5)
...
static int ngx_libc_cdecl ngx_http_upstream_sns_chash_cmp_points(const void *one, 
    const void *two);
static ngx_int_t ngx_http_upstream_sns_chash_init(ngx_http_upstream_srv_conf_t *uscf,
    ngx_http_upstream_rr_peers_t *tmp_peers);
static ngx_int_t ngx_http_upstream_sns_del_chash_peer(
    ngx_http_upstream_srv_conf_t *uscf);


static int ngx_libc_cdecl
ngx_http_upstream_sns_chash_cmp_points(const void *one, const void *two)
{
...
static ngx_int_t
ngx_http_upstream_sns_chash_init(ngx_http_upstream_srv_conf_t *uscf,
    ngx_http_upstream_rr_peers_t *tmp_peers)
{
    size_t                                    new_size;
    ...
    ngx_qsort(points->point,
              ...
              ngx_http_upstream_sns_chash_cmp_points);
...
static ngx_int_t
ngx_http_upstream_sns_del_chash_peer(ngx_http_upstream_srv_conf_t *uscf)
{
    size_t                                    host_len, port_len;
    u_char                                   *host, *port, c;
    uint32_t                                  hash, base_hash;
